...
Gmail 발송 전 한국어 비즈니스 이메일의 오류를 잡아내고 수정하는 Chrome 확장기능입니다.

## 주요 기능

- Gmail 작성 창에 **[검토하기]** 버튼 추가
- 9개 카테고리 오류 감지: 수신자 호칭, 중복 표현, 띄어쓰기, 오타/맞춤법, 경어체 불일치, 누락 요소, 어색한 표현, 조사 오류, 문단 구분
- Google Gemini API를 활용한 실시간 스트리밍 검토
- 수정된 전체 이메일을 모달로 미리보기
- 변경 사항별 체크박스로 선택적 반영
- 작성자 문체 유지 (LLM이 작성한 티가 나지 않도록)
- 답장/전달 시 수신자 정보 및 원본 메일 컨텍스트 자동 추출
- 검토 카테고리 ON/OFF, 사용자 규칙 추가, LLM 모델 선택
- 자주 하는 실수 TOP 3 인사이트
...
```bash
git clone https://github.com/pengdo-myorange/BeforeSend.git
```
...
4. 클론한 `BeforeSend` 폴더를 선택합니다.
5. 확장기능 목록에 **보내기 전에**가 나타나면 설치 완료입니다.

### 3. API 키 설정

이 확장기능은 Google Gemini API를 사용합니다. 사용자 본인의 API 키가 필요합니다.
...
| 항목 | 설명 |
|------|------|
| LLM 모델 | Gemini 3.1 Flash Lite (기본), Gemini 3 Flash, Gemini 3.1 Pro 중 선택 |
| 검토 카테고리 | 9개 카테고리 개별 ON/OFF |
| 사용자 규칙 | 추가 검토 규칙을 자유롭게 입력 (예: "귀사" 대신 "귀 법인"으로 수정) |
...
자세한 내용은 [개인정보처리방침](privacy.html)을 참고하세요.
